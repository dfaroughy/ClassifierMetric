...
class JetNetDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        particles, jet = self.apply_preprocessing(datasets[idx], stats=self.summary_statistics['all'])  
        output['particle_features'] = particles[:, :-1]
    ...
    def apply_preprocessing(self, sample, stats):
        sample = PreprocessData(data=sample, stats=stats, compute_jet_features=self.compute_jet_features)
        sample.preprocess(methods=self.preprocess)
